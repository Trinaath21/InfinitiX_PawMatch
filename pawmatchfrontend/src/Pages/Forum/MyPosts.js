...
const ContentComponent = () => {
  ...
  const [currentCategory, setCurrentCategory] = useState('myPosts');
...
useEffect(() => {
  fetchMyPosts(); // Load my posts on mount
}, []);

  // Fetch post basd on category general or my posts
  useEffect(() => {

    if (currentCategory === 'myPosts') {
        setData(myPosts); // Show user-specific posts by default
      } else if (currentCategory === 'general') {
        setData(allPosts); // Show all posts if the category changes
      }
