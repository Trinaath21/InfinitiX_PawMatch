...
const PublicLostPetListings = () => {
    ...
    const fetchReportData = async () => {
      try {
        const response = await axios.post(
          "http://localhost:8000/api/getAllReports",
          {
        ...
        const uniqueDistricts = [
          ...new Set(response.data.data.map((report) => report.district)),
        ];
  ...
  const filteredReports = reportData.filter((report) => {
    ...
    const matchesKeyword = searchKeyword
      ? report.pet_name.toLowerCase().includes(searchKeyword.toLowerCase()) ||
        report.breed.toLowerCase().includes(searchKeyword.toLowerCase())
      : true;

    const matchesDateRange =
      dateRange[0] && dateRange[1]
        ? moment(report.dateLost).isBetween(
                ...
                <Tooltip title="Click to view more">
                  ...
                  >
                    <Card.Meta
                      title={
                        <Typography.Text
                          ellipsis={{ tooltip: report.pet_name }}
                        >
                          {report.pet_name}
                        </Typography.Text>
                      }
                      description={
                        <Typography.Text
                          ellipsis={{ tooltip: `Breed: ${report.breed}` }}
                        >
                          Breed: {report.breed}
                        </Typography.Text>
                      }
